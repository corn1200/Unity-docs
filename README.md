...
# 1. 리지드바디
## 개요
***리지드바디(Rigidbody)*** 는 게임 오브젝트의 ***물리적 동작***을 가능하게 하는 주요 컴포넌트다.    
리지드바디가 연결된 오브젝트는 ***중력에 즉시 반응***한다.    
***Collider*** 컴포넌트도 하나 이상 추가할 경우 게임 오브젝트가 당하는 ***충돌***에 의해 움직인다.

***Rigidbody*** 컴포넌트는 연결된 게임 오브젝트의 이동을 제어하므로 스크립트에서 포지션과 회전 같은 ***트랜스폼 프로퍼티를 변경***하여 오브젝트를 이동하려고 해선 안된다.   
그 대신 ***Forces***를 가하여 게임 오브젝트를 밀고 ***물리 엔진***이 결과를 계산하도록 해야 한다.
...
이 프로퍼티는 ***물리 엔진의 제어 대상에서 컴포넌트를 제외***하고, 스크립트에서 컴포넌트가 물리 힘의 영향을 받지 않고 움직이도록 하기 위해 사용할 수 있다.    
오브젝트에 대한 물리를 켜고 끌 수 있게 스크립트에서 ***Is Kinematic*** 값을 변경할 수 있지만, ***성능 오버헤드가 발생***하므로 신중하게 사용해야 한다.

## 프로퍼티
***Rigidbody***는 ***GameObject***가 물리 제어로 동작하게 한다.     
리지드바디는 ***힘과 토크***를 받아 오브젝트가 사실적으로 움직이도록 해준다.  
...
빨리 움직이는 오브젝트의 리지드바디에서 ***연속 충돌 검사***를 활성화하면 문제를 해결할 수 있다.  
...
## 올바른 크기의 사용
게임 오브젝트의 메시 크기는 ***리지드바디의 질량***보다 훨씬 중요하다.    
리지드바디가 천천히 움직이거나, 떠 있거나, 제대로 충돌하지 않는 등 예상한 대로 동작하지 않으면 메시 에셋의 스케일을 조정해 보아야 한다.     
***Unity의 기본 단위 스케일은 1 unit = 1 미터***이므로 임포트한 메시의 스케일은 유지되며 물리 연산에 적용된다.    
예를 들어 무너지고 있는 고층 빌딩은 장난감 블록으로 쌓아 올린 탑과는 붕괴되는 양상이 상당히 다르다.     
그러므로 다른 크기의 오브젝트를 정확한 스케일로 모델링해야 한다.

사람을 모델링한다면 Unity에서 모델의 신장이 약 2미터인지 확인해야 한다.     
오브젝트가 올바른 크기인지 여부를 확인하려면 기본 큐브와 비교해볼 수 있다.    
큐브는 ***GameObject > 3D Object > Cube*** 로 생성할 수 있다.     
***큐브의 높이는 정확히 1미터***이기 때문에 사람은 큐브보다 두 배 정도 커야 한다.     

메시 자체를 조정할 수 없다면 ***Project View***를 선택하고 메뉴에서 ***Assets > Import Settings...***에서 특정 메시 에셋의 균등한 스케일을 바꿀 수 있다.    
여기에서는 스케일을 변경하고 메시를 다시 임포트할 수 있다.

게임 오브젝트를 다른 스케일로 인스턴스화할 필요가 있는 경우 ***트랜스폼의 스케일 축 값을 조정***해도 된다.    
이 때 단점은 ***오브젝트가 인스턴스화될 때 물리 시뮬레이션이 더 많은 작업을 해서 게임 퍼포먼스를 저하***시킬 수 있다는 점이다.    
상당한 소실까지는 아니지만 다른 두 가지 옵션으로 스케일을 마무리하는 것만큼 효율적이지는 않다.  
또한 ***불균등한 스케일은 부모가 사용될 때 원하지 않는 동작을 수행***하게 할 수 있다.     
그렇기 때문에 언제나 오브젝트를 모델링 애플리케이션에 맞게 정확한 스케일로 생성하는 것이 좋다.

## 기타
- 두 리지드바디의 상대 ***Mass***는 둘이 서로 충돌했을 때 어떻게 반응할지를 결정한다.
- 한 리지드바디의 ***Mass***를 다른 리지드바디보다 크게 만들어도 자유 낙하 시 더 빨리 낙하하지 않는다.    
이 경우 ***Drag***를 사용해야 한다.
- ***Drag*** 값이 작으면 오브젝트가 무거워 보이고 값이 크면 가벼워 보인다.    
***Drag***의 일반적인 값은 .001(단단한 금속 덩어리)과 10(깃털) 사이다.
- 오브젝트의 ***Transform*** 컴포넌트를 직접 조작하면서도 물리 연산이 필요하다면 리지드바디를 추가하고 ***키네마틱***으로 만든다.
- ***Transform*** 컴포넌트를 통해 게임 오브젝트를 이동시키면서도 ***충돌/트리거 메세지***를 받고자 한다면 이동하는 오브젝트에 리지드바디를 추가해야 한다.
- ***Angle Drag***를 무한대로 설정하는 것 만으로는 오브젝트의 회전을 멈추게할 수는 없다.

# 2. 콜라이더
***Collider*** 컴포넌트는 ***물리적 충돌***을 위해 리지드바디에 함께 추가해야 하는 또 다른 유형의 컴포넌트이며, 게임 오브젝트의 모양을 정의한다.  
두 개의 리지드바디가 서로 충돌하더라도 ***두 오브젝트 모두 콜라이더가 추가되어 있지 않으면 물리 엔진은 충돌을 연산하지 않는다***.    
콜라이더가 없는 리지드바디는 물리 시뮬레이션 동안 서로를 지나쳐가기만 한다.     
보이지 않는 콜라이더는 게임 오브젝트의 메시와 완전히 똑같을 필요는 없다.    
메시의 대략적인 근사치로도 효율적일 때가 많으며, 게임플레이에서 구별하기 어렵다.

![리지드바디](/img/rigidbody1.png)
> 콜라이더는 리지드바디의 물리적 경계를 정의한다.

가장 간단한(그리고 프로세서에 부하를 주지 않는) 콜라이더는 기본 콜라이더 타입이다.  
3D에서는 박스 콜라이더, 스피어 콜라이더, 캡슐 콜라이더가 바로 이 타입이다.  
복합 콜라이더를 만들기 위해 이러한 콜라이더를 단일 게임 오브젝트에 원하는 만큼 추가할 수 있다.
...
## 트리거(Triggers)
스크립팅 시스템은 충돌이 일어나는 시점을 감지하고 ***OnCollisionEnter*** 함수를 사용하여 행동을 제어할 수 있다.   
또한, 단순히 하나의 콜라이더가 충돌을 일으키지 않고 다른 콜라이더의 공간에 들어가는 것을 물리 엔진이 감지하도록 할 수도 있다.   
콜라이더를 ***트리거(Trigger)***(***Is Trigger*** 프로퍼티 사용)로 설정하면 이 콜라이더는 단단한 오브젝트로 동작하지 않으며 그저 ***다른 콜라이더가 통과하도록 허용***한다.   
...
충돌 이벤트가 발생했을 때의 동작을 정의하기 위해 원하는 대로 코드를 작성할 수 있다.     
예를 들어, 차가 장애물에 부딪쳤을 때 충돌 음향 효과를 재생하도록 할 수 있다.

충돌이 감지되었을 때 가장 먼저 일어나는 물리 업데이트는 ***OnCollisionEnter*** 함수 호출이다.     
접촉이 지속되는 상황에서는 업데이트 동안 ***OnCollisionStay*** 함수가 호출되며, 마지막으로 이 접촉 상태가 종료되었음을 나타내는 ***OnTriggerExit*** 함수를 호출한다.    

노멀 비트리거 충돌에서는 충돌에 관여하는 최소한 하나의 오브젝트가 비키네마틱 리지드바디를 가져야 한다는 추가 조건이 있다(즉, ***Is Kinematic***이 꺼져 있어야 한다).  
두 오브젝트가 모두 키네마틱 리지드바디이면 ***OnCollisionEnter*** 등의 함수가 호출되지 않는다.    
트리거 충돌의 경우 이 제약이 적용되지 않으며, 따라서 키네마틱과 비키네마틱 리지드바디 모두 트리거 콜라이더에 들어가면 ***OnTriggerEnter***가 호출된다.

## 콜라이더 상호작용
콜라이더는 ***Rigidbody*** 컴포넌트의 환경 설정에 따라 서로 다르게 상호작용한다.      
정적 콜라이더(즉, 리지드바디가 전혀 붙어있지 않음), 리지드바디 콜라이더, 키네마틱 리지드바디 콜라이더 등 세 가지 중요한 환경설정이 있다.

### 정적 콜라이더
***Rigidbody*** 컴포넌트 없이도 콜라이더를 게임 오브젝트에 추가하여 씬의 바닥, 벽, 기타 고정된 요소를 생성할 수 있다.     
이러한 콜라이더는 ***정적 콜라이더***라고 부른다.     
이와 반대로, 리지드바디가 있는 게임 오브젝트의 콜라이더는 ***동적 콜라이더***라고 부른다.     
정적 콜라이더는 동적 콜라이더와 상호작용이 가능하지만, 정적 콜라이더에는 리지드바디가 없기 때문에 충돌에 리스폰스하여 움직이지는 않는다.    
정적 콜라이더는 항상 같은 위치에 그대로 있으면서 절대로 움직이지 않는 레벨 지형에 주로 사용된다.    
***리지드바디 오브젝트가 다가오면 정적 콜라이더와 충돌하지만 정적 콜라이더가 움직이지는 않는다***.

***특정한 경우 물리 엔진은 절대 움직이지 않는 정적 콜라이더에 대해 최적화한다***.     
예를 들어 정적 콜라이더 위에 놓인 차량은 이 정적 콜라이더를 움직여도 고정된 상태를 유지한다.    
***물리 엔진 계산 속도에 별다른 영향을 주지 않고도 런타임 시점에 정적 콜라이더를 활성화 또는 비활성화하거나 이동시킬 수 있다***.  
또한 스케일이 왜곡되지 않고 일정한 경우에 한하여 정적 메시 콜라이더를 안전하게 스케일할 수 있다.

### 리지드바디 콜라이더
리지드바디 콜라이더는 ***노멀 비키네마틱 리지드바디와 콜라이더가 있는 게임 오브젝트***다.   
리지드바디 콜라이더는 ***물리 엔진에 의해 완전히 시뮬레이션***되며 ***스크립트로부터 적용되는 충돌과 힘에 반응***한다.  
다른 오브젝트(정적 콜라이더 포함)에 충돌할 수 있으며 물리를 사용하는 게임 내 가장 흔히 사용되는 콜라이더 설정이다.

### 키네마틱 리지드바디 콜라이더
키네마티 리지드바디 콜라이더는 ***콜라이더와 키네마틱 리지드바디***가 있는(즉, 리지드바디의 ***IsKinematic*** 프로퍼티가 활성화됨) 게임 오브젝트이다.   
키네마틱 리지드바디 오브젝트는 ***Transform*** 컴포넌트를 수정하여 스크립트를 통해 움직일 수 있지만, ***비키네마틱 리지드바디***처럼 충돌이나 힘에 반응하지는 않는다.     
키네마틱 리지드바디는 ***상황에 따라 움직이거나 활성화/비활성화되지만 그 외 상황에서는 정적 콜라이더처럼 동작하는 콜라이더에 사용***해야 한다.    
예를 들어, 평소에는 움직이지 않는 물리 장애물처럼 동작하지만 필요할 때는 열 수 있는 미닫이문과 같은 경우에 사용한다.    
정적 콜라이더와 달리, 움직이는 키네마틱 리지드바디는 다른 오브젝트에 마찰을 가하며 따라서 접촉이 일어나면 다른 리지드바디를 ***"깨우게"*** 된다.

키네마틱 리지드바디 콜라이더는 움직이지 않고 있을 때에도 정적 콜라이더와는 다르게 동작한다.     
예를 들어, 콜라이더가 트리거로 설정되어 있으면 스크립트에서 트리거 이벤트를 받기 위해 리지드바디를 추가해야 한다.   
트리거가 중력의 영향을 받거나 물리 영향을 받지 않도록 하고 싶으면 해당 리지드바디의 ***IsKinematic*** 프로퍼티를 설정하면 된다.

***Rigidbody*** 컴포넌트는 ***IsKinematic*** 프로퍼티를 사용하여 언제든 노멀 동작 또는 키네마틱 동작으로 변경할 수 있다.

좋은 예로 ***"래그돌"*** 효과가 있다.     
래그돌의 경우 캐릭터는 일반적으로 애니메이션 하에서 움직이지만 폭발이나 큰 충돌 시 물리적으로 던져진다.     
캐릭터의 팔다리에는 ***IsKinematic***이 디폴트로 활성화되어 있는 각각의 ***Rigidbody*** 컴포넌트가 주어진다.    
***IsKinematic***이 활성화되어 있는 동안은 팔다리가 애니메이션에 의해 일반적으로 움직이다가, 비활성화되는 순간 즉시 물리 오브젝트처럼 동작하기 시작한다.  
이 시점에서 충돌력 또는 폭발력으로 인해 캐릭터는 팔다리가 현실처럼 마구 흔들리면서 날아가게 된다.

## 충돌 액션 매트릭스
두 오브젝트가 충돌하면 오브젝트 리지드바디의 환경설정에 따라 여러 다른 스크립트 이벤트가 발생할 수 있다.    
아래의 표는 오브젝트에 있는 컴포넌트에 따라 어떤 이벤트 함수가 호출되는지를 구체적으로 나타낸다.    
일부 조합의 경우 두 오브젝트 중에서 하나만 충돌의 영향을 받으며, 일반적으로는 ***Rigidbody*** 컴포넌트가 없는 오브젝트에는 물리가 적용되지 않는 것이 원칙이다.

### ``충돌 시 충돌 검사가 되고 메세지가 전송된다``
||정적콜라이더|리지드바디 콜라이더|키네마틱 리지드바디 콜라이더|정적 트리거 콜라이더|리지드바디 트리거 콜라이더|키네마틱 리지드바디 트리거 콜라이더|
|---|---|---|---|---|---|---|
|정적 콜라이더||Y|||||
|리지드바디 콜라이더|Y|Y|Y||||
|키네마틱 리지드바디 콜라이더||Y|||||
...
# 3. 물리 머티리얼
***Physic Material***은 충돌하는 오브젝트의 ***마찰 또는 바운스 효과***를 조정하는 데 사용한다.

물리 머티리얼을 만들려면 메뉴 바에서 ***Assets > Create > Physic Material***을 선택한 다음 물리 머티리얼을 프로젝트 뷰에서 씬의 ***Collider***로 드래그한다.

## 프로퍼티
![물리 머티리얼 프로퍼티](/img/physicmaterial0.png)

|프로퍼티:|기능:|
|---|---|
|Dynamic Friction|이미 움직이고 있을 때 사용되는 마찰이다. 일반적으로 0과 1 사이의 값이다. 값이 0이면 얼음처럼 미끄러워지고 1이면 오브젝트가 큰 힘이나 중력에 밀리지 않는 한 아주 빨리 정지한다.|
|Static Friction|오브젝트가 표면 위에 가만히 놓여 있을 때 사용되는 마찰이다. 일반적으로 0과 1 사이의 값이다. 0이면 얼음처럼 미끄러워지고, 1이면 오브젝트를 움직이기가 매우 힘들어진다.|
|Bounciness|표면이 얼마나 탄성이 있는지 나타낸다. 값이 0이면 오브젝트가 바운스하지 않고 값이 1이면 오브젝트가 에너지 손실 없이 바운스한다. 다만 시뮬레이션에 적응 양의 에너지가 더해질 수 있는 근사값이 있을 수 있다.|
|Friction Combine|충돌하는 두 오브젝트의 마찰이 합쳐지는 방법이다.|
|- Average|두 마찰 값의 평균이 사용된다.|
|- Minimum|두 값 중 작은 값이 사용된다.|
|- Maximum|두 값 중 큰 값이 사용된다.|
|- Multiply|두 마찰 값을 서로 곱한다.|
|Bounce Combine|충돌하는 두 오브젝트의 탄성을 합치는 방법이다. 마찰 결합 모드와 동일한 모드가 있다.|

## 상세
마찰은 표면이 서로 미끄러지지 않도록 하는 정도이다.     
이 값은 오브젝트를 쌓으려고 할 때 중요하다.     
마찰의 두 가지 형태는 ***동적 마찰***과 ***정지 마찰***이다.    
***Static friction***은 오브젝트가 가만히 정지해 있는 경우에 사용한다.    
이 마찰은 오브젝트가 움직이기 시작하는 것을 방지한다.   
오브젝트에 충분히 큰 힘이 가해지면 오브젝트가 움직이기 시작한다.    
이 시점에 ***Dynamic Friction***이 작용하기 시작한다.    
이제 ***Dynamic Friction***이 오브젝트가 서로 접촉하는 동안 오브젝트의 속도를 줄이려고 한다.

두 바디가 접촉되어 있는 경우 선택된 모드에 따라 동일한 탄성 및 마찰 효과가 두 바디에 모두 적용된다.     
접촉하는 ***두 콜라이더에 서로 다른 결합 모드가 설정된 특별한 경우***가 있다.     
이 경우 ***우선 순위가 가장 높은 함수가 사용***된다.  
우선 순위는 ***Average < Minimum < Multiply < Maximum*** 이다.    
예를 들어 한 머티리얼에 ***Average***가 설정되어 있지만 다른 머티리얼에는 ***Maximum***이 설정되어 있는 경우 우선 순위가 높은 ***Maximum*** 결합 함수가 사용된다.

***Nvidia PhysX*** 엔진이 사용하는 마찰 모델은 시뮬레이션 성능과 안정성을 최적화하도록 튜닝되어 있고, 현실 세계의 물리를 유사하게 표현하지 않을 수도 있다.    
특히 단일 점보다 큰 접촉 표면(예: 서로 겹치게 쌓은 박스 2개)은 접점이 2개 있는 것으로 계산되고, 현실 세계의 물리보다 마찰력이 두배다.   
이 경우 더 사실적인 결과를 얻기 위해 마찰 계수에 0.5를 곱할 수 있다.

동일한 로직이 탄성 모델에도 적용된다.   
***Nvidia PhysX***는 위치 보정 등 다양한 시뮬레이션 정보로 인해 완벽한 에너지 보존을 보장하지 않는다.     
예를 들어 중력에 영향을 받는 오브젝트의 탄성값이 1이고 탄성이 1인 지표에 충돌할 경우 오브젝트가 최초 위치보다 더 높은 위치에 다다를 것이라고 예상할 수 있다.

# 4. 시네머신
![시네머신](/img/cinemachine0.png)

시네머신은 유니티 카메라를 작동하기 위한 모듈 모음이다.     
시네머신은 대상 추적, 구성, 블렌딩, 샷 자르기의 복잡한 수학과 논리 문제를 해결한다.     
개발 중에 발생하는 수동 조작 및 스크립트 수정을 크게 줄일 수 있도록 설계되었다.

모듈의 절차적 특성으로 인해 시네머신은 버그를 방지할 수 있다.   
애니메이션, 차량 속도, 지형 또는 씬의 다른 게임 오브젝트를 변경하는 등의 조정을 수행할 때 시네머신은 최적의 장면을 만들기 위해 동작을 동적으로 조정한다.  
예를 들어, 캐릭터가 오른쪽이 아니라 왼쪽으로 돌았다고 해서 카메라 스크립트를 다시 쓸 필요는 없다.

FPS, 3인칭, 2D, 횡스크롤, 탑다운, RTIS 등 모든 장르에 걸쳐 실시간으로 동작이 가능하다.  
필요한 만큼 여러 장면을 얻을 수 있고 모듈 시스템을 통해 정교한 행동을 구성할 수 있다.

시네머신은 타임라인, 애니메이션 및 포스트 프로세싱을 강력하게 보완하는 역할을 하며 다른 Unity 도구와 잘 호환된다.

## 버츄얼 카메라
시네머신은 새로운 카메라를 만드는 대신 여러 장의 촬영을 위해 단일 유니티 카메라를 가리킨다.     
***버츄얼 카메라***를 이용하여 이러한 장면을 구성한다.    
버츄얼 카메라는 유니티 카메라를 움직이고 회전하며 설정을 제어한다.

***버츄얼 카메라는 유니티 카메라와 별개의 게임 개체이며 독립적으로 동작한다***.     
이들은 서로 중첩되어있지 않다.  
예를 들어, ***씬(Scene)*** 은 다음과 같다.

![버츄얼 카메라](/img/cinemachine1.png)

버츄얼 카메라가 수행하는 주요 작업은 다음과 같다.
- 유니티 카메라를 ***씬(Scene)*** 에 배치한다.
- 유니티 카메라가 무언가를 조준하도록 한다.
- 유니티 카메라에 절차적 노이즈를 추가한다.     
노이즈는 핸드헬드 효과나 차량 흔들림 등을 시뮬레이션한다.

시네머신은 버츄얼 카메라를 많이 만들 것을 권장한다.     
버츄얼 카메라는 프로세스 비용이 거의 소모되지 않도록 설계되었다.    
***씬(Scene)*** 이 성능에 민감한 경우에는 언제든지 필수 가상 카메라를 제외한 모든 가상 카메라를 비활성화하여 최상의 성능을 발휘한다.

버츄얼 카메라는 언제든지 유니티 카메라를 제어할 수 있고, 이 버츄얼 카메라는 ***라이브*** 상태가 된다.  
이 규칙의 예외는 한 버츄얼 카메라에서 다른 버츄얼 카메라로 ***블렌드가 발생하는 경우***다.    
블렌드 중에는 두 개의 버츄얼 카메라가 모두 ***라이브*** 상태가 된다.

## 시네머신 브레인
***시네머신 브레인은 유니티 카메라 자체의 컴포넌트다***.  
시네머신 브레인은 ***씬(Scene)*** 에 있는 모든 활성 버츄얼 카메라를 모니터링한다.  
다음 라이브 버츄얼 카메라를 지정하려면 원하는 버츄얼 카메라의 게임 오브젝트를 활성화하거나 비활성화한다.    
그러면 시네머신 브레인이 ***라이브 버츄얼 카메라와 우선 순위가 높거나 같고 가장 최근에 활성화된 버츄얼 카메라를 선택***한다.  
이전의 버츄얼 카메라와 새로운 버츄얼 카메라의 장면 사이를 잘라내거나 블렌드한다.

> 참고: 시네머신 브레인을 사용하여 동적인 게임 이벤트에 실시간으로 대응할 수 있고, ***우선순위를 조작하여 게임 로직이 카메라를 제어할 수 있게 해준다***.     
이것은 액션이 항상 예측할 수 없는 라이브 게임 플레이에 특히 유용한다.   
***타임라인***을 사용하여 컷신과 같은 예측 가능한 상황에서 카메라의 연속 행동을 지시할 수 있다.   
타임라인은 정확한 프레임 단위의 카메라 제어를 제공하기 위해 ***시네머신 브레인 우선 순위 시스템을 오버라이드***한다.

## 이동 및 조준
버츄얼 카메라의 ***Body*** 프로퍼티를 사용하여 ***씬(Scene)*** 에서 이동하는 방법을 지정할 수 있고, ***Aim*** 속성을 사용하면 회전하는 방법을 지정할 수 있다.

버츄얼 카메라에는 두 가지 대상이 있다.
- ***Follow*** 대상은 버츄얼 카메라가 함께 이동할 게임 오브젝트를 지정한다.
- ***Look At*** 대상은 바라볼 게임 오브젝트를 지정한다.

시네머신에는 이동과 조준을 제어하기 위한 다양한 절차 알고리즘이 포함되어 있다.  
각 알고리즘은 특정 문제를 해결하며 특정 요구에 맞게 알고리즘을 사용자 정의할 수 있는 속성을 가지고 있다.    
시네머신은 이러한 알고리즘을 ***CinemachineComponent*** 오브젝트로 구현한다.    
***CinemachineComponent*** 클래스를 사용하여 사용자 정의 이동 또는 조준 동작을 구현한다.

***Body*** 속성은 ***씬(Scene)*** 에서 버츄얼 카메라를 이동하기 위해 다음과 같은 절차적 알고리즘을 제공한다.
- Transposer: 선택적 댐핑을 사용하여 ***Follow target***에 고정된 관계로 이동한다.
- Do Nothing: 버츄얼 카메라를 이동하지 않는다.
- Framing Transposer: 선택적 댐핑과 함께 ***Follow target***에 고정된 화면 공간 관계로 이동한다.
- Orbital Transposer: 선택적으로 플레이어 입력을 허용하고 ***Follow target***에 대한 가변적인 관계로 이동한다.
- Tracked Dolly: 사전 정의된 경로를 따라 이동한다.
- Hard Lock to Target: ***Follow target***과 동일한 위치를 사용한다.

***Aim*** 속성은 버츄얼 카메라가 ***Target***을 향하도록 회전하기 위해 다음과 같은 절차적 알고리즘을 제공한다.
- Composer: 구성 제약 조건을 사용하여 카메라 프레임에 ***Look At*** 대상을 유지한다.
- Group Composer: 카메라 프레임에 여러 개의 ***Look At*** 대상을 유지한다.
- Do Nothing: 버츄얼 카메라를 회전시키지 않는다.
- POV: 사용자의 입력에 따라 버츄얼 카메라를 회전한다.
- Same As Follow Target: 카메라의 회전을 ***Follow Target***의 회전으로 설정한다.
- Hard Look At: ***Look At*** 대상을 카메라 프레임 중앙에 유지한다.

## 샷 구성
***Framing Transposer***, ***Composer*** 및 ***Group Composer*** 알고리즘은 카메라 프레임에서 샷을 작성할 수 있는 영역을 정의한다.
- Dead zone: 시네머신이 ***Target***을 유지하는 프레임 영역이다.
- Soft zone: ***Target***이 프레임의 이 영역에 들어오면 카메라는 ***Target***을 다시 데드존에 넣기 위해 방향을 조정한다.    
댐핑 설정에 지정된 시간에 따라 이 작업을 천천히 또는 빠르게 수행한다.
- Screen: 데드존 중앙의 화면 위치다.    
0.5는 화면의 중심이다.
- Damping: 실제 카메라 조작자가 무거운 물리적 카메라를 조작하는 동안 발생하는 지연을 시뮬레이션한다.    
댐핑은 카메라가 ***Target***을 추적하는 동안 ***Target***이 소프트 존에 진입할 때 카메라가 반응하는 속도를 빠르게 또는 느리게 지정한다.     
카메라를 빠르게 움직이거나 조준하여 ***Target***을 데드존에 유지하려면 작은 숫자를 사용하여 보다 반응성이 뛰어난 카메라를 시뮬레이션할 수 있다.   
숫자가 클수록 더 무거운 카메라를 시뮬레이션한다.  
값이 클수록 시네머신이 ***Target***을 소프트 존으로 더 많이 진입시킬 수 있다.

***Game Window Guides***는 이러한 영역을 상호작용적이고 시각적으로 표시해 준다.   
이런 가이드는 ***Game view***에 색칠된 영역으로 나타난다.

![시네머신](/img/cinemachine2.png)

선명한 영역은 데드존을 나타내고, 파란색 영역은 소프트 존을 나타낸다.    
소프트존과 데드존의 위치는 ***screen*** 위치를 나타낸다.  
빨간색 영역은 통과 불가 영역을 나타내며, 대상은 이 영역에 들어가지 않는다.  
노란색 사각형은 ***Target***을 나타낸다.

이 영역을 조정하여 다양한 카메라 동작을 얻을 수 있다.   
이렇게 하려면 ***Game view***에서 가장자리를 끌거나 인스펙터 창에서 속성을 편집한다.  
예를 들어, 더 큰 댐핑 값을 사용하여 더 크고 무거운 카메라를 시뮬레이션하거나 소프트존 및 데드존을 확대하여 카메라 프레임 중앙에 대상 모션에 영향을 받지 않는 영역을 만든다.     
애니메이션 사이클과 같이 카메라가 대상이 조금만 움직일 경우 대상을 추적하지 않도록 하려면 이 기능을 사용한다.

## 노이즈를 사용하여 카메라 흔들림 시뮬레이션
실제 물리적 카메라는 종종 무겁고 번거롭다.  
그것들은 카메라 조작자에 의해 손에 잡히거나 움직이는 차량과 같은 불안정한 물체에 장착된다.
***Noise*** 프로퍼티를 사용하여 영화적 효과를 위한 실제 품질을 시뮬레이션한다.    
예를 들어, 달리는 캐릭터를 따라갈 때 카메라 흔들림을 추가하여 플레이어를 액션에 몰입시킬 수 있다.

프레임 업데이트 때마다 시네머신은 ***카메라의 움직임과 별도로 노이즈를 추가***하여 타겟을 따라간다.   
***노이즈는 이후 프레임에서 카메라의 위치에 영향을 주지 않는다***.    
이러한 분리는 ***댐핑과 같은 프로퍼티가 예상대로 작동하도록 보장***한다.
