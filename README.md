...
3. 업데이트: MonoBehaviour 컴포넌트를 사용하면 FixedUpdate(), Update(), LateUpdate() 등의 업데이트 함수를 활용하여 게임 오브젝트의 동작을 프레임마다 업데이트할 수 있다.    
각 업데이트 함수는 정해진 순서에 따라 호출되며, 컴포넌트의 업데이트는 이러한 함수들을 통해 이루어진다.
4. 상호작용: 컴포넌트는 게임 오브젝트의 상호작용을 구현하기 위해 사용된다.  
예를 들어, 콜라이더(Collider) 컴포넌트는 충돌 검사를 통해 다른 오브젝트와의 상호작용을 감지할 수 있다.  
또한 스크립트 컴포넌트를 사용하여 사용자 입력에 따라 오브젝트의 동작을 제어할 수 있다.
5. 제어와 행위 분리: 컴포넌트 기반 설계의 장점은 제어와 행위를 분리할 수 있다는 점이다.     
즉, 각 컴포넌트는 독립적으로 작동하며, 필요에 따라 추가/제거/교체가 가능하다.   
이로 인해 코드의 재사용성이 증가하고 개발자들은 높은 수준의 모듈화를 이뤄낼 수 있다.
6. 컴포넌트 간 통신: 컴포넌트들은 서로간에 통신을 할 수 있으며, 이를 통해 게임 오브젝트의 동작을 복잡하게 조합하고 상호작용할 수 있다.  
